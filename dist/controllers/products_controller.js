...
class ProductController {
    constructor() {
        this.createProduct = async (req, res) => {
            ...
            // 1. Insert product
            const productResult = await pool.query(`INSERT INTO products (name, price, quantity, image, owner_id)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`, [
                product.name,
                product.price,
            ...
            for (const option of product.options) {
                const optionResult = await pool.query(`INSERT INTO product_options (product_id, name)
         VALUES ($1, $2)
         RETURNING id`, [productId, option.name]);
                const optionId = optionResult.rows[0].id;
                // 3. Insert each value for the option
                for (const value of option.values) {
                    await pool.query(`INSERT INTO product_option_values (option_id, value)
